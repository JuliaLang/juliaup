...
fn parse_nightly_channel_or_id(channel: &str) -> Option<String> {
    // Directly compile the Regex without using Lazy
    let nightly_re =
...
pub fn channel_to_name(channel: &String) -> Result<String> {
    ...
    let version = if let Some(version_prefix) = parse_nightly_channel_or_id(channel) {
        // Only add "-latest" if the version prefix is not empty
        if version_prefix.is_empty() {
            "latest".to_string() // If version_prefix is empty, use "latest"
        } else {
            format!("{}-latest", version_prefix) // Otherwise, format with the version
        }
    } else {
        channel.to_string() // If no match is found, use the original channel
    };
