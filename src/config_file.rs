...
#[derive(Serialize, Deserialize,Clone)]
pub struct JuliaupConfigChannel {
...
pub fn load_config_db() -> Result<JuliaupConfig> {
    ...
    let file = match File::open(&path) {
        ...
        Err(_) => return Ok(JuliaupConfig { default: "release".to_string(), installed_versions: HashMap::new(), installed_channels: HashMap::new() })
    };
...
pub fn save_config_db(config_data: &JuliaupConfig) -> Result<()> {
    ...
    let file = File::create(&path)
        .with_context(|| format!("Failed to open configuration file '{}' for saving.", display))?;
