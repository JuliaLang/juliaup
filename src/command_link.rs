...
pub fn run_command_link(channel: String, file: String, args: Vec<String>) -> Result<()> {
    let config_file = open_mut_config_file()
        .with_context(|| "`link` command failed to open configuration file.")?;
    
    let (mut config_data, file_lock) = load_mut_config_db(&config_file)
    ...
    save_config_db(&config_file, config_data, file_lock)
