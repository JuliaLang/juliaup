...
fn check_channel_uptodate(channel: &str, current_version: &str, versions_db: &JuliaupVersionDB) -> Result<()> {
	...
	if latest_version != current_version {
		...
		println!();
		println!("to install Julia {} and update the `{}` channel to that version.", latest_version, channel);
	}
    Ok(())
}

fn get_julia_path_from_channel(versions_db: &JuliaupVersionDB, config_data: &JuliaupConfig, channel: &str, juliaupconfig_path: &Path) -> Result<(PathBuf,Vec<String>)> {
    ...
    match channel_info {
        JuliaupConfigChannel::LinkedChannel {command, args} => return Ok((PathBuf::from(command), args.clone().unwrap())),
        JuliaupConfigChannel::SystemChannel {version} => {
            ...
            let absolute_path = juliaupconfig_path.parent().unwrap().join(path).join("bin").join(format!("julia{}", std::env::consts::EXE_SUFFIX)).normalize()?;
...
fn main() -> Result<()> {
    ...
    let (julia_path, julia_args)  = get_julia_path_from_channel(&versiondb_data, &config_data, &julia_channel_to_use, &juliaupconfig_path)?;
    ...
    std::process::Command::new(julia_path).args(&new_args).status()?;
